<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>header</title>
</head>
<body>
<header style="position: fixed; top: 0px; width: 100%; height: 40px;
				background-color: blue; color: white; text-align: center; line-height: 40px; 
				font-size: 30px"> 쇼핑몰 회원관리 ver 1.0 </header>
				
<nav style="position: fixed; top:40px; width: 100%; height: 20px; 
			background-color: lightblue; color: white; line-height: 20px; padding-left: 20px"> 
			<a href="<%= request.getContextPath() %>/member/join.jsp">  회원등록 </a> &nbsp;&nbsp;	
			<a href="<%= request.getContextPath() %>/member/list.jsp">회원목록조회/수정 </a>&nbsp;&nbsp; 
			<a href="<%= request.getContextPath() %>/sale/salary.jsp">회원매출조회 </a>&nbsp;&nbsp;&nbsp;&nbsp;	
			<a href="<%= request.getContextPath() %>">홈으로.</a>
</nav>
</body>
</html>